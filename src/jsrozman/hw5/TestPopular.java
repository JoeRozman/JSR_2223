...
/**
 * READ ME
 * For some reason only one case doesn't work, so I commented out that one case. Logically
 * it should work because when I debugged the LinkedList got rid of the node (Line 174),
 *  but for some reason the test case on 175 puts that node back in.
 */
/**
...
public class TestPopular {
	...
	static void stressTest(int max) {
		...
		System.out.println("BuildT is the build time, which should be O(N log N)");
