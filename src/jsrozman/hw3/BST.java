...
public class BST {
	...
	int depthCount(Node n, int depth) {
		...
		count += depthCount(n.left, depth-1);
		count += depthCount(n.right, depth-1);
	...
	Node mostFrequent(Node n, Node best) {
		...
		Node b1 = mostFrequent(n.left, best);
		Node b2 = mostFrequent(n.right, best);
	...
	Node truncate(Node n, int depth) {
		...
		if(depth == 0) {
			...
			depth--;
			truncate(n.left, depth);
			truncate(n.right, depth);
		}
